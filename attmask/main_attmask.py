...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default=".", type=str, help='Path to save logs and checkpoints into generated subdiretory. See --folder_name')
    parser.add_argument('--folder_name', default=None, type=str, help='Subdirectory to save checkpoints and logs. If None, defaults so <timestap>-<rnd_adjective>-<rnd_petname>')
...
@torch.no_grad()
def eval_writerretrieval(model, dataloader, log_as=""):
    model.eval()
    features_per_page = defaultdict(list)
    writer_of_page = {}
    page_descriptors = torch.zeros((1182, 384)).cuda()
    for batch in tqdm(dataloader, total=len(dataloader), desc="Validation"):
        
        images, writers, pages = batch
        writers = writers.cuda()
        pages = pages.cuda()
        images = images.cuda()
        features = model(images, return_all_tokens=False, return_attention=False)  
        
        for feat, page, writer in zip(features, pages, writers):
            page_descriptors[page] += feat
            #features_per_page[page.item()].append(feat)
            writer_of_page[page.item()] = writer
            
    
    # page_descriptors = torch.zeros((len(features_per_page), feat.shape[-1])).cuda()
    # for k,v in features_per_page.items():
    #     page_descriptors[k] = torch.mean(torch.stack(v, dim=0), dim=0)
    # page_descriptors = torch.stack([
    #     torch.mean(torch.stack(feature_list, dim=0), dim=0)
    #     for page, feature_list in features_per_page.items()]).cuda()
    
    # page_writerids = torch.stack(list(writer_of_page.values())).cuda()
    page_writerids = torch.stack([writer_of_page[i] for i in range(len(writer_of_page))]).cuda()
    
    top1, mAP = compute_top1_and_map(page_descriptors, page_writerids)
    model.train()
    return {f'{log_as}_mAP': mAP.item(), f'{log_as}_Top1': top1.item()}
...
def train_attmask(args):
    ...
    match args.augmentation:
        ...
        case 'morph':
            transform_train = DataAugmentationMorph(
                ...
                drop_rate= 0 #0.5 if args.patch_size==16 else 0.75,
            )
    ...
    dataset_val = icdar2017patcher(
        os.path.join(args.data_path), # here we use the path of the train split of imagenet
        window_size=224,
        transform=transforms.ToTensor(),
        at_least_white=0,
        overlap=0,
    )
    print(f"There are {len(dataset_val)} Validation Images.")
    ...
    if args.sanity_check:
            # ========= eval student ============
            retrieval_results_student = eval_writerretrieval(student.module.backbone, data_loader_val, log_as="student")
            print(retrieval_results_student)
            
            # # ========= eval teacher ============
            # retrieval_results_teacher = eval_writerretrieval(teacher_without_ddp.backbone, data_loader_val, log_as="teacher")
            # print(retrieval_results_teacher) 
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.eval_every == 0 or epoch == args.epochs - 1:
            # ========= eval student ============
            retrieval_results_student = eval_writerretrieval(student.module.backbone, data_loader_val, log_as="student")
            log_stats.update(retrieval_results_student)
            
            # ========= eval teacher ============
            retrieval_results_teacher = eval_writerretrieval(teacher_without_ddp.backbone, data_loader_val, log_as="teacher")
            log_stats.update(retrieval_results_teacher)   
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    # ps = student.module.backbone.patch_embed.patch_size
    # avg2d = torch.nn.AvgPool2d(ps, ps)
    # n_fg = 1
    # n_fg /= ps**2
    # drop_rate=0.5
    ...
    for it, (images, labels) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            if args.pred_shape == 'rand' or args.pred_shape == 'block':
                ...
                teacher_output, teacher_attention = teacher(images[:args.global_crops_number], return_attention = True)#, keep_ids=ids[:args.global_crops_number])
            ...
            student_output = student(images[:args.global_crops_number], mask=masks)#, keep_ids=ids)
            
            # get local views
            student.module.backbone.masked_im_modeling = False
            student_local_cls = student(images[args.global_crops_number:])[0] if len(images) > args.global_crops_number else None #keep_ids=ids[args.global_crops_number]: <- inside the forward
...
class DataAugmentationMorph(object):
    ...
    def __call__(self, image):
        ...
        avgs = [(self.avg2d(im).mean(0)>=self.n_fg).float().flatten() for im in crops]
...
if __name__ == '__main__':
    ...
    #     args.folder_name = datetime.datetime.now().strftime(f'%Y%m%d-%H-%M-%S-{petname.Generate()}')
    #args.output_dir = str(Path(args.output_dir) / args.folder_name)
